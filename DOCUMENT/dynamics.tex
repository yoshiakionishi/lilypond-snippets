...
\textcolor{red}{\textbf{NB: } Revision (August 19, 2026) has been carried out in modifying where the default parenthesized dynamics will be placed. Before, the parenthesized dynamics with whiteout would be placed on top of the constante line after the line break, but at a position that exposes the beginning part of the constante line. I fixed it so that the dynamics would hide the beginning part of the constante line.  
...
\textcolor{red}{\textbf{NB: }Significantly updated on December 23 and 24, 2025, with new commands being added on January 15 2026.}
...
If you wish to use circled tips, instead of using \verb|\override Hairpin.circled-tip = ##t|, write:

\begin{verbatim}
NOTE \o \flaredCresc \< NOTE(S) \DYNAMIC OR \! \flaredDecresc \> NOTE(S) \o
\end{verbatim}

\textcolor{red}{Update: January 15, 2026 } If you are using the flared crescendo/diminuendo to a note that also has \verb|\after|, use:
...
Considering that flared hairpins are often used in tandem with ordinary hairpins, I have embedded \verb|\tweak Hairpin.stencil = #flared-hairpin-new| in \verb|\flaredCresc| and \verb|\flaredDecresc|. Therefore, it is not necessary to write an \verb|\override| command to change the stencil used.

\textcolor{red}{Update: December 23-24, 2025 } \verb|\flaredCresc| and \verb|\flaredDecresc| accepts a number as an optional argument. This functionality has been added to allow the adjustment of flaring end of the hairpin, as well as the horizontal placement of the small piece of the arrival hairpin when \verb|\override to-barline| is set \verb|##f|. While I have striven to set the default setting in such a way that these micro-adjustments are not necessary, it seemed helpful to add this functionality when further refinement is needed. Think of this as an \verb|X-offset| for the arrival point. See \textbf{Code}. 


\textcolor{red}{Update: January 15, 2026 } I have discovered that \verb|\flaredCresc| and \verb|\flaredDecresc| do not work when it is applied to a note that also has \verb|\after| preceding it, e.g. when the hairpin is terminated within the given note. In order to make such a case work, I have created additional commands \verb|\flaredCrescOverride| and  \verb|\flaredDecrescOverride|, which use \verb|\once \override| commands instead of \verb|\tweak|. These commands also accept an optional argument that behaves like \verb|X-offset|. 
...
\textcolor{red}{\textbf{NB: }Significantly updated on December 23, 2025 and cosmetic alteration undertaken on December 24, 2025, as well as new commands being added on January 15 2026.}
...
Considering that flared hairpins are often used in tandem with ordinary hairpins, I have embedded \verb|\tweak Hairpin.stencil = #flared-hairpin-curvy| in \verb|\flaredCrescCurvy| and \verb|\flaredDecrescCurvy|. Therefore, it is not necessary to write an \verb|\override| command to change the stencil used.

\textcolor{red}{Update: December 23-24, 2025 } \verb|\flaredCresc| and \verb|\flaredDecresc| accepts a number as an optional argument. This functionality has been added to allow the adjustment of flaring end of the hairpin, as well as the horizontal placement of the small piece of the arrival hairpin when \verb|\override to-barline| is set \verb|##f|. While I have striven to set the default setting in such a way that these micro-adjustments are not necessary, it seemed helpful to add this functionality when further refinement is needed. Think of this as an \verb|X-offset| for the arrival point. See \textbf{Code}. 


\textcolor{red}{Update: January 15, 2026 } I have discovered that \verb|\flaredCrescCurvy| and \verb|\flaredDecrescCurvy| do not work when it is applied to a note that also has \verb|\after| preceding it, e.g. when the hairpin is terminated within the given note. In order to make such a case work, I have created additional commands \verb|\flaredCrescCurvyOverride| and  \verb|\flaredDecrescCurvyOverride|, which use \verb|\once \override| commands instead of \verb|\tweak|. These commands also accept an optional argument that behaves like \verb|X-offset|.
