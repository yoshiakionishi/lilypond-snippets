...
\section {Jet Whistle (for flute) (deprecated)}
\label{sec:jet_whistle_1}
...
\textcolor{red}{
This snippet has been deprecated. The snippet itself works for \Verb|2.26.0|; however, readers are advised to use \hyperref[sec:jet_whistle_2]{Version 2} of the implementation (initially released Aug. 10, 2025), which will continue to be updated.
}


Implementation of the jet whistle, as described in \textit{The Techniques of Flute Playing} by Carin Levine and Christina Mitropoulos-Bott.\autocite[18]{RN1695} \par\hfill\par
\subsection{Grammar}
\begin{verbatim}
\jet NOTE #X-length
\end{verbatim}
...
\section {Jet Whistle (for flute) (Version 2)}
\label{sec:jet_whistle_2}
\lilypondfile[staffsize=24]{/jet_v2.ly}
\hfill
...
This is an improved version of the \hyperref[sec:jet_whistle_1]{original version of the jet whistle implementation.} This version adds the functionality to adjust the height, as well as the width of the jet whistle sign.
...
\subsection{Description}
This snippet is hardly my own idea, as I largely quoted this technique from one of the snippets available on LilyPond Wiki, formerly LSR.\footnote{See: \url{https://wiki.lilypond.community/wiki/Creating_harp_glissandi}} However, I list it here because: 
\begin{enumerate} 
...
\end{enumerate}
See \hyperref[sec:noteheadless_discussion]{Discussion}. 
...
\end{Verbatim}
\label{noteheadless_discussion}
\subsection{Discussion}

As discussed in the \href{https://lilypond.org/doc/v2.26/Documentation/learning/the-stencil-property}{Learning Manual}, overriding \Verb|NoteHead.stencil| to \Verb|##f| will cause an error when the processing requires the dimensions of the NoteHead. I now recommend: \par

\Verb|\once \override NoteHead.stencil = #point-stencil| \par

There is also another special stencil, namely \Verb|#empty-stencil|, but this appears to cause the processing to assume that there is no pitch assigned to the particular note. In the aforementioned snippet found on LilyPond Wiki, it causes the glissando line to only reach and emerge out of the middle of the five-line staff.
...
\section {Parenthesized Group of Noteheads with Arrowed Arcs}
\lilypondfile[staffsize=24]{/parenthesis_arcarrow.ly}
\hfill

\subsection{Description}
	Implementation of parentheses enclosing a group of noteheads, accompanied by arrowed arcs. This function takes advantage of manipulating the \verb|\parenthesize| function, and can be applied to both regular and grace notes. \par
	There are two options that can be set: 
	\begin{enumerate}
	\item The arrowed arcs can be adjusted \`a la \verb|extra-offset| (i.e., inputting the values using a pair of values)
	\item You can choose the parentheses not to be printed, so that only arrowed arcs are displayed.
	\end{enumerate}
	A group of small noteheads, accompanied by arrowed arcs, suggests that such a group of notes are to be repeatedly played as fast as possible. This notation can be found in pieces such as \textit{Sequenza II} for harp by Luciano Berio. \par
	In addition to the examples seen above, see \hyperref[sec:repeat_notepatterns_afap]{\textbf{Repeat Note Patterns As Fast As Possible}} in \chapref*{ch:combinations}.
...
\textbf{NB} \par
\begin{itemize}
\item \verb|WIDTH| specifies the width of the arrowed arc.
\item \verb|HEIGHT| specifies the height of the arrowed arc, i.e. how tall the arc will be in the middle of the line.
\item \verb|ARCANGLE| specifies how much the arrowed arc should be rotated. A positive value will rotate the arc counterclockwise, while a negative value will rotate it clockwise.
\item \textbf{OPTIONAL:} \verb|OFFSET| can be used to offset the arrowed arc. If you opt to specify this argument, enter a value in a pair of numbers, e.g. \verb|#'(3 . -2)|. 
\item \textbf{OPTIONAL:} \verb|PARENTHESIS?| can be specified if you opt to make the parenthesis disappear. \verb|##f| will cause only the arrowed arc to be shown. 
\item Importantly, when there are other objects (markups and spanners shown alongside the arrowed arcs) being in place alongside this object, it may cause the other objects to appear farther from the staff line. Setting \verb|outside-staff-property| to \verb|##f| for these other objects, either via \verb|\tweak| or \verb|\override| should help avoid the problem, but if the problem persists, moving them using \verb|extra-offset| is always an option. 
\end{itemize}
...
\end{Verbatim} 
\subsection{Discussion}
See \hyperref[sec:repeat_notepatterns_afap]{\textbf{Repeat Note Patterns As Fast As Possible}} in \chapref*{ch:combinations} for an applied use of this function. \par
\hyperref[sec:toc]{\textbf{Table of Contents}}
...
\hfill
\subsection{Description}
Implementation of the tongue ram notation, as described in \textit{The Techniques of Flute Playing} by Carin Levine and Christina Mitropoulos-Bott.\autocite[28]{RN1695} See also: \href{https://www.flutexpansions.com/tongue-ram}{FluteXpansions}.\par
\textcolor{red}{Please note that the scheme function I had in previous versions of this document is no longer functional and deprecated. See \hyperref[sec:tongue_ram_discussion]{\textbf{Discussion}} for more details.}
...
\begin{verbatim}
 ...
 \tweak NoteHead.text
 \markup \musicglyph "noteheads.s2triangle" LOW_PITCH>
\end{verbatim}
...
\end{Verbatim}
\label{sec:tongue_ram_discussion}
\subsection{Discussion}
