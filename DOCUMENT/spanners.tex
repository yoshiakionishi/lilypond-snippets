...
\section {Arpeggio with Arrowhead and Vertical Hairpins}
\label{sec:arpeggioArrowheadVerticalHairpin}
\lilypondfile[staffsize=20,hpadding=4,voffset=4pt]{/arpeggioArrowheadVerticalHairpin.ly}
\hfill

\subsection{Description}
Implementation of vertical hairpins associated with the directional arpeggios. LilyPond already provides functionality for the arpeggios with arrowheads.\footnote{\url{https://lilypond.org/doc/v2.26/Documentation/internals/arpeggio_002dinterface}} However, I could not find a convenient snippet for the vertical hairpins to be used in tandem with such arpeggios. 

In fact, I decided to take the route of drawing vertical hairpin with \verb|\path| function within \verb|\markup|. Should you so desire, you could tweak the value within it to further customize the opening width of the hairpin, for example. 

Note that there are two functions, one to be used for single-staff arpeggios, another one for arpeggio that spans over multiple staves. This is because for the latter, the context must be set to \Verb|Score| instead of \Verb|Staff|. 

\subsection{Grammar}

\begin{verbatim}
 \arpeggioWithVerticalHairpinSingleStaff 
 	#ARPEGGIO_DIRECTION #CRESCENDO_DIRECTION "DYNAMIC_TOP" "DYNAMIC_BOTTOM" #HAIRPIN_X-OFFSET
...
\textbf{NB} 
\begin{itemize}
\item \Verb|#ARPEGGIO_DIRECTION| sets the direction of the arrowhead. \Verb|#UP| places an arrowhead at the top of the arpeggio, while \Verb|#DOWN| at the bottom.
\item \Verb|#CRESCENDO_DIRECTION| defines the direction of the \textit{crescendo as a shape}. To avoid any confusion, it is encouraged to think of this argument independently from the arpeggio direction you previously declared, and to think in a purely visual way (i.e. \say{How does this crescendo look on the paper, is it looking like \Verb|V| or \rotatebox[origin=c]{180}{\Verb|V|}?}).  For example, \Verb|#UP| places a hairpin that looks like \Verb|V| while \Verb|#DOWN| will place a hairpin that looks like \rotatebox[origin=c]{180}{\Verb|V|}.
\item \Verb|"DYNAMIC_TOP"| and \verb|"DYNAMIC_BOTTOM"| determine which dynamic indications to place at the top and the bottom of the hairpin, respectively. Note that you have to put the dynamics in quotation marks. If you desire no dynamic marking, for either end or both of the hairpin, use \verb|" "|, i.e. surround a space with quotation marks. Declaring \verb|""| will result in warning messages.
\item \Verb|#HAIRPIN_X-OFFSET| determines the distance between the hairpin and the arpeggio. Normally setting this argument to \Verb|#0| should pose no problem.
\end{itemize}
...
\section {Dashed Flat Slurs}
\label{sec:dashed_flat_slurs}
\hfill
\lilypondfile[staffsize=24,hpadding=4,voffset=4pt]{/dashed_flat_slur.ly}
\hfill

\subsection{Description}

See \hyperref[sec:flat_slurs]{\textbf{Flat Slurs}} for the ordinary non-dashed version.

Because the \hyperref[sec:flat_slurs]{\textbf{Flat Slurs}} is based on a rewritten stencil, further overrides within \verb|Slur|, such as \href{https://lilypond.org/doc/v2.24/Documentation/notation/expressive-marks-as-curves#Making-slurs-with-complex-dash-structure}{Making slurs with complex dash structure} will not work. This particular implementation addresses the issue of achieving dashed lines for the flat slurs. Here, three different stencils are created, defined as variables, then combined using \verb|ly:stencil-add|.

See \hyperref[sec:flat_slurs]{\textbf{Flat Slurs}} for the ordinary non-dashed version, as well as \hyperref[sec:flat_slurs_discussion]{\textbf{Discussion}} for hints on tweaking.
...
This flat slur requires careful tweaking in order to make it look optimal on the score. This is discussed further in \hyperref[sec:flat_slurs_discussion]{\textbf{Discussion}}.

See also \hyperref[sec:dashed_flat_slurs]{\textbf{Dashed Flat Slurs}}.
